...
export const BROKERS = [
  ...
  },
  {
    ...
    apiNote:
      "Integration depends on Groww’s published access; may use statement export as fallback.",
  },
  {
    id: "upstox",
    name: "Upstox",
...
function looksLikeHeader(cells) {
  ...
  return /symbol|instrument|ticker|product|qty|quantity|avg|average|cost|ltp|name|company/.test(
    joined,
  );
}
 ...
 */
export function parseHoldingsCsv(raw) {
  const errors = [];
  const lines = raw
    .trim()
    .split(/\r?\n/)
  ...
  let colQty = idx(
    (h) => h.includes("qty") || h.includes("quantity") || h.includes("units"),
  );
  let colAvg = idx((h) =>
  ...
  let colName = idx(
    (h) =>
      /^(name|company|stock\s*name)$/i.test(h) ||
      (h.includes("name") && !h.includes("user")),
  );
  ...
  if (colSymbol < 0) {
    errors.push(
      "Could not detect a Symbol/Instrument column. Use a header row.",
    );
  ...
  dataLines.forEach((line, i) => {
    ...
    if (!sym || !/^[A-Z0-9.&-]+$/.test(sym)) {
      errors.push(
        `Row ${i + 1 + (hasHeader ? 1 : 0)}: invalid symbol/instrument "${cells[colSymbol]}"`,
      );
      return;
    }
    ...
    const name = colName >= 0 && cells[colName] ? cells[colName] : undefined;
