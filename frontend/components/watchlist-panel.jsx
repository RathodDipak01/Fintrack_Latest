...
function WatchlistGrid({ data }) {
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {data.map((item) => (
        <div
          key={item.symbol}
          className="rounded-lg border border-white/10 bg-white/[0.035] p-5 transition hover:border-ai/30 hover:bg-white/[0.06]"
        >
          <div className="flex items-center justify-between">
            ...
            <div className="grid h-11 w-11 place-items-center rounded-md bg-panel2 font-semibold text-white">
          ...
          <div className="mt-8 flex items-end justify-between">
            ...
            <span
              className={
                item.change.startsWith("-") ? "text-loss" : "text-profit"
              }
            >
              {item.change}
            </span>
...
function WatchlistTable({ data, onSort, sortField, sortDirection }) {
            ...
            >
              <td className="rounded-l-lg px-3 py-4">
                <div className="flex items-center gap-3">
                  <div className="grid h-9 w-9 place-items-center rounded-md bg-panel2 text-xs font-semibold text-white">
                    {item.symbol.slice(0, 2)}
                  </div>
              ...
              <td className="max-w-[280px] px-3 py-4 text-slate-400">
                {item.alert}
              </td>
              ...
              <td
                className={`rounded-r-lg px-3 py-4 text-right font-semibold ${item.change.startsWith("-") ? "text-loss" : "text-profit"}`}
              >
                {item.change}
              </td>
            </tr>
      ...
      <SectionHeader
        ...
        action={
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              className="rounded-md bg-ai px-3 py-2 text-sm font-semibold text-white"
            >
