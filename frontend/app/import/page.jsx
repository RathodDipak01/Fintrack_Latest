...
export default function ImportPortfolioPage() {
    ...
    try {
      ...
      try {
        ...
        if (e.message.includes("Authentication") || e.message.includes("401")) {
          setServerMsg(
            "Authentication required. Please login first to save your portfolio.",
          );
        } else {
  ...
  const connectBroker = async (brokerId) => {
    ...
    try {
      ...
      setBrokerStatus(
        data.message +
          (data.documentationHint ? ` — ${data.documentationHint}` : ""),
      );
    } catch {
    ...
    <AppShell>
      <div>
        ...
        <p className="mt-4 max-w-3xl text-slate-400">
          ...
          <strong className="text-slate-300">Angel One</strong>,{" "}
          <strong className="text-slate-300">Zerodha</strong>,{" "}
          <strong className="text-slate-300">Groww</strong>, and{" "}
          <strong className="text-slate-300">Upstox</strong> APIs when
          connected, or from a{" "}
          <strong className="text-slate-300">CSV export</strong> of your
          positions. Live API sync is wired in stages; CSV works end-to-end for
      ...
      <GlassCard className="p-6">
        ...
        <p className="mb-6 max-w-3xl text-sm leading-6 text-slate-400">
          Each integration uses the broker&apos;s official API (e.g. Zerodha
          Kite Connect, Angel SmartAPI, Upstox developer API). Groww may require
          export workflows until programmatic access matches your account.
          Buttons below confirm the planned flow; credentials are never stored
          in this demo.
        </p>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {BROKERS.map((b) => (
            <div
              key={b.id}
              className="flex flex-col rounded-lg border border-white/10 bg-white/[0.035] p-4"
            >
      ...
      <GlassCard className="p-6">
        ...
        <p className="mb-4 max-w-3xl text-sm text-slate-400">
          Use columns such as <span className="text-slate-300">Instrument</span>
          , <span className="text-slate-300">Product</span>,{" "}
          <span className="text-slate-300">Quantity</span>,{" "}
          <span className="text-slate-300">Average Price</span>, optional{" "}
          <span className="text-slate-300">LTP</span> and{" "}
          <span className="text-slate-300">Name</span>. Or omit headers and
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_1.2fr]">
          <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border border-dashed border-white/20 bg-white/[0.02] px-4 py-10 transition hover:border-ai/40">
            ...
            <span className="mt-1 text-xs text-slate-500">
              Broker exports often work after minor column cleanup
            </span>
            <input
              type="file"
              accept=".csv,text/csv"
              ...
              <table className="w-full min-w-[640px] border-separate border-spacing-y-1 text-left text-sm">
                ...
                <tbody>
                  {preview.map((h, i) => (
                    <tr
                      key={`${h.symbol}-${i}`}
                      className="rounded-md bg-white/[0.035] text-slate-300"
                    >
            ...
            <div>
              ...
              {serverSummary ? (
                <ul className="mt-2 text-xs text-slate-400">
                  ...
                  <li>
                    Invested (approx): ₹
                    {serverSummary.investedApprox.toLocaleString("en-IN")}
                  </li>
                  <li>
                    Unrealized P&amp;L vs cost (approx): ₹
                    {serverSummary.unrealizedApprox.toLocaleString("en-IN")}
                  </li>
                </ul>
              ) : null}
            </div>
