...
export default function SuggestionsPage() {
  ...
  const positiveOutcomes = suggestionHistory.filter((s) =>
    s.outcome.startsWith("+"),
  ).length;
  const riskWarnings = suggestionHistory.filter(
    (s) => s.type === "RISK ALERT",
  ).length;
    ...
    <AppShell>
      <div>
        ...
        <p className="mt-4 max-w-3xl text-slate-400">
          A history of AI suggestions, confidence levels, triggers and follow-up
          outcomes. This app does not place orders.
        </p>
      </div>
        ...
        <div className="space-y-3">
          {suggestionHistory.map((item) => (
            <div
              ...
              className="grid gap-3 rounded-lg border border-white/10 bg-white/[0.035] p-4 sm:grid-cols-[110px_1fr_auto] sm:items-center"
            >
              <Pill
                tone={
