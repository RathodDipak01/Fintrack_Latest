...
import { CircleDollarSign, LineChart, Plus, Search } from "lucide-react";
...
import { analystRatings, companyInsights, keyEvents, sentimentDetails, shareholdingPatterns, stockTechnicalsDetail, stockFundamentalsDetail, stockFinancialsDetail } from "@/lib/portfolio-data";

export default function StocksPage() {
  return (
    <AppShell>
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-profit">Stock Detail</p>
        <h1 className="mt-2 text-3xl font-bold text-white md:text-5xl">Adani Power Ltd</h1>
        <p className="mt-4 max-w-3xl text-slate-400">Mobile-inspired stock screen with overview, live events, shareholding, analyst rating and action controls.</p>
      </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="border-b border-white/10 p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h2 className="mt-1 text-2xl font-semibold text-white">₹248.95 <span className="text-base text-profit">+4.2%</span></h2>
              </div>
            ...
            <div className="mt-5"><MobilePriceChart /></div>
        ...
        <div className="space-y-5">
          <GlassCard className="p-6">
            <SectionHeader eyebrow="Candlestick" title="Technical chart" action={<Pill tone="profit">UP 81%</Pill>} />
            <CandleProxyChart />
          </GlassCard>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
            ...
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="border-b border-white/10 bg-white/5 text-slate-400">
                <tr>
                  ...
                  {["'26", "'25", "'24", "'23", "'22", "'21"].map((year, idx) => (
                    <th key={year} className={`px-4 py-3 font-medium ${idx === 0 ? "text-ai" : ""}`}>{year}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-white/10">
                {shareholdingPatterns.map((row) => (
                  <tr key={row.investor}>
                    <td className="px-4 py-4 font-medium text-white whitespace-nowrap">{row.investor}</td>
                    {row.values.map((val, idx) => (
                      <td key={idx} className={`px-4 py-4 whitespace-nowrap ${idx === 0 ? "text-white font-medium" : ""}`}>{val}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
        ...
        <GlassCard className="p-6">
          ...
          <p className="text-sm leading-7 text-slate-300">Adani Power, a key player in India's power sector, was incorporated in 1996 and founded by Gautam Adani. The company is headquartered in Ahmedabad, Gujarat.</p>
      ...
      <section className="grid gap-5 xl:grid-cols-3">
        <GlassCard className="p-6">
          ...
          <p className="mb-5 text-sm leading-7 text-slate-300">{stockTechnicalsDetail.summary}</p>
          <div className="divide-y divide-white/10 rounded-lg border border-white/10">
            {stockTechnicalsDetail.metrics.map((metric) => (
              <div key={metric.label} className="flex justify-between px-4 py-3 text-sm">
                <span className="text-slate-400">{metric.label}</span>
                <div className="text-right">
                  <div className="font-semibold text-white">{metric.value}</div>
                  <div className="text-[10px] uppercase text-slate-500">{metric.note}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <GlassCard className="p-6">
          ...
          <div className="space-y-5">
            {stockFundamentalsDetail.groups.map((group) => (
              <div key={group.title} className="rounded-lg border border-white/10 bg-white/[0.035] p-4">
                <h3 className="mb-3 text-sm font-semibold text-white">{group.title}</h3>
                <div className="space-y-2">
                  {group.rows.map((row) => (
                    <div key={row.label} className="flex justify-between border-b border-white/10 pb-2 text-sm last:border-b-0 last:pb-0">
